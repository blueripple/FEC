...
main = do
  ...
  let
    clientEnv = mkClientEnv manager FEC.baseUrl
    runServant :: forall a . ClientM a -> IO (Either ClientError a)
...
-- find expenditures for a candidate to a certain payee whether they are from campaign committee, independent or party.  And collect the involved committees.
    query = do
      ...
      sequence
        (   (\x -> getCandidateSpending x electionYear payees)
        <$> V.toList candidates
        )
  doIf queryData $ do
  ...
  spending :: [CandidateSpending] <- fmap (fromMaybe [])
